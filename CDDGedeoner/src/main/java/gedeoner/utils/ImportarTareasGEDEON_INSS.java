...
public class ImportarTareasGEDEON_INSS extends AbstractExcelReader{
	private static final String DG_Factory = "FACTDG05";
	protected static IEntityLogic peticionesEntidad, subdireccionEntidad, 
		servicioEntidad, aplicativoEntidad, tiposPeticionEntidad;
	
	public static final String ORIGEN_FROM_SG_TO_CDINSS = "INSS", ORIGEN_FROM_CDISM_TO_AT = "CDINSS", ORIGEN_FROM_AT_TO_DESARR_GESTINADO = "SDG";
	private static final String CDINSS = "Centro de Desarrollo del INSS", CONTRATO_7201_17G_L1 = "7201 17G L1";
		...
		if (centroDestino != null) {
			if (centroDestino.contentEquals(DG_Factory)){
				...
				if (fsetUnidadOrigen == null){
					servicioAtiendePeticion = ORIGEN_FROM_SG_TO_CDINSS;
				}else{
			...
			try {
				...
				for (final FieldViewSet registro : filas) {
					...
					if (existenColl != null && !existenColl.isEmpty()){
						...
						if ( CDINSS.equals(centroDestinoPadre)){
							numImportadas += linkarPeticionesDeSGD_a_CD(peticionEnBBDD, idsHijas_);
						}else if ( centroDestinoPadre.contains(CONTRATO_7201_17G_L1)){
	...
	public static void main(String[] args){
		try{
			...
			final ImportarTareasGEDEON_INSS importadorGEDEONes = new ImportarTareasGEDEON_INSS(dataAccess_);
