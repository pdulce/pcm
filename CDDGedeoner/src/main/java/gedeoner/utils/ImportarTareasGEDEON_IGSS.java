...
public class ImportarTareasGEDEON_IGSS extends AbstractExcelReader{
	private static final String DG_Factory = "FACTDG06";
	...
	public static final String ORIGEN_FROM_SG_TO_CDINSS = "IGSS", ORIGEN_FROM_CDISM_TO_AT = "CDIGSS", ORIGEN_FROM_AT_TO_DESARR_GESTINADO = "SDG";
	private static final String CDIGSS = "Centro de Desarrollo del IGSS", CONTRATO_DG = "Desarrollo Gestionado 7206/18 L3";
			...
			try {
				...
				for (final FieldViewSet registro : filas) {
					...
					if (existenColl != null && !existenColl.isEmpty()){
						...
						if ( CDIGSS.equals(centroDestinoPadre)){
							numImportadas += linkarPeticionesDeSGD_a_CD(peticionEnBBDD, idsHijas_);
						}else if ( centroDestinoPadre.contains(CONTRATO_DG)){
