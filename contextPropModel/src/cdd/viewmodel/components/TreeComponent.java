...
public class TreeComponent extends AbstractComponent {
	...
	private void drawFolder(int grandGrandParentFolderId, int grandParentFolderId, final StringBuilder xml, final Element folder,
		...
		node_ = new StringBuilder(node_.toString().replaceFirst(NAME_, folder.getAttribute(IViewModel.NAME_ATTR)));
		if (folder.hasAttribute(IViewModel.LINK_ATTR)) {
			final StringBuilder href_ = new StringBuilder();
			drawHref(href_, folder.getAttribute(IViewModel.LINK_ATTR));
		...
		for (int j = 0; j < nodesChildrenCount; j++) {
			...
			if (!leafNode.getNodeName().equals(IViewModel.LEAF_ELEMENT)) {
				continue;
			}
			final Element leaf = (Element) leafNode;
			final String linkUri = leaf.getAttribute(IViewModel.LINK_ATTR);
			...
			final String name = leaf.getAttribute(IViewModel.NAME_ATTR);
		...
		for (int k = 0; k < nodesChildrenCount; k++) {
			...
			if (!folderChildNode.getNodeName().equals(IViewModel.FOLDER_ELEMENT)) {
				continue;
			}
		...
		try {
			...
			if (!treesCached.containsKey(keyComposed)) {
				...
				for (int i = 0; i < treeElem.getChildNodes().getLength(); i++) {
					...
					if (!folderNode.getNodeName().equals(IViewModel.FOLDER_ELEMENT)) {
						continue;
					}
					Element folder = (Element) folderNode;
					final String profOfEntry = folder.getAttribute(IViewModel.PROFILE_ATTR);
