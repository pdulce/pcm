package pcm.context.logicmodel;
...
import pcm.context.viewmodel.definitions.FieldViewSetComparator;
		...
		try {
			for (final FieldViewSet fieldViewSet : entities.getFieldViewSets()) {
				...
				final boolean borradoLogico = fieldViewSet.getEntityDef().getFieldSet()
						.get(this.getDaoRef().getAuditFieldset().getProperty(IViewMetamodel.FEC_BAJA)) != null;
