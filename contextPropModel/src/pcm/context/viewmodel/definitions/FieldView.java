...
import pcm.comunication.actions.validators.ByteValidator;
import pcm.comunication.actions.validators.DateValidator;
import pcm.comunication.actions.validators.DoubleValidator;
import pcm.comunication.actions.validators.IntegerValidator;
import pcm.comunication.actions.validators.LongValidator;
import pcm.comunication.actions.validators.RelationalAndCIFValidator;
import pcm.comunication.actions.validators.StringValidator;
...
public class FieldView implements IFieldView, Serializable {
	...
	public String getValueOfOption(final String dictionary, final String valueOfOption) {
		...
		try {
			...
			BasePCMServlet.log.log(Level.SEVERE, "Error", cfExc);
			return null;
		}
	}
	...
	private void setCommonAttrs(final Element nodeField) {
		if (nodeField.hasAttribute(IViewMetamodel.NAME_ATTR)) {
			if (!nodeField.hasAttribute(ContextProperties.MAPPING_TO_ATTR)) {
				this.userDefined = true;
			}
			this.setUserName(nodeField.getAttribute(IViewMetamodel.NAME_ATTR));
		...
		if (optionValuesSet.getLength() == 1) {
			...
			if (optionSet.hasAttribute(IViewMetamodel.ENTITYMODEL_ATTR)) {
				final String entityOptionsName = optionSet.getAttribute(IViewMetamodel.ENTITYMODEL_ATTR);
	...
	public final boolean checkDataType(final RequestWrapper request_, final String val_, final boolean validacionObligatoria,
		...
		try {
			...
			BasePCMServlet.log.log(Level.SEVERE, InternalErrorsConstants.CHECK_TYPES_ERROR, exc);
			return false;
		}
	}
