...
public class TreeComponent extends AbstractComponent {
	...
	private void drawFolder(int grandGrandParentFolderId, int grandParentFolderId, final StringBuilder xml, final Element folder,
		...
		node_ = new StringBuilder(node_.toString().replaceFirst(NAME_, folder.getAttribute(IViewMetamodel.NAME_ATTR)));
		if (folder.hasAttribute(IViewMetamodel.LINK_ATTR)) {
			final StringBuilder href_ = new StringBuilder();
			drawHref(href_, folder.getAttribute(IViewMetamodel.LINK_ATTR));
		...
		for (int j = 0; j < nodesChildrenCount; j++) {
			...
			if (!leafNode.getNodeName().equals(IViewMetamodel.LEAF_ELEMENT)) {
				continue;
			}
			final Element leaf = (Element) leafNode;
			final String linkUri = leaf.getAttribute(IViewMetamodel.LINK_ATTR);
			...
			final String name = leaf.getAttribute(IViewMetamodel.NAME_ATTR);
		...
		for (int k = 0; k < nodesChildrenCount; k++) {
			...
			if (!folderChildNode.getNodeName().equals(IViewMetamodel.FOLDER_ELEMENT)) {
				continue;
			}
		...
		try {
			...
			if (!treesCached.containsKey(keyComposed)) {
				...
				for (int i = 0; i < treeElem.getChildNodes().getLength(); i++) {
					...
					if (!folderNode.getNodeName().equals(IViewMetamodel.FOLDER_ELEMENT)) {
						continue;
					}
					Element folder = (Element) folderNode;
					final String profOfEntry = folder.getAttribute(IViewMetamodel.PROFILE_ATTR);
			...
			BasePCMServlet.log.log(Level.SEVERE, InternalErrorsConstants.XML_TREE_GENERATION, parqExc);
	...
	public String toXHTML(final RequestWrapper request, final IDataAccess dataAccess_, boolean submitted) throws DatabaseException {
		try {
			...
			BasePCMServlet.log.log(Level.SEVERE, InternalErrorsConstants.XML_TREE_GENERATION, exc);
			return null;
		}
	}
