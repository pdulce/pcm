...
import pcm.context.viewmodel.components.controls.html.GenericHTMLElement;
import pcm.context.viewmodel.components.controls.html.GenericInput;
...
import pcm.context.viewmodel.definitions.FieldViewComparator;
...
import pcm.context.viewmodel.definitions.OptionsSelection;
...
public class PaginationGrid extends AbstractComponent {
	...
	public PaginationGrid(final String service_, final Element viewElement_, final String formId_, final ContextApp appCtx_,
	...
	protected final void initFieldViewSets(final Element element, final RequestWrapper request, IDataAccess dataAccess)
		...
		try {
			...
			for (int i = 0; i < nodesFViewSetCount; i++) {
				...
				String nameSpaceEntity = attrs.getNamedItem(IViewMetamodel.NAMESPACE_ENTITY_ATTR).getNodeValue();
				String entityNameInMetamodel = attrs.getNamedItem(IViewMetamodel.ENTITYMODEL_ATTR).getNodeValue();
				final Collection<IFieldView> coleccionFieldViews = new ArrayList<IFieldView>();
				final NodeList nodosFieldViews = fieldViewSetNode.getElementsByTagName(IViewMetamodel.FIELDVIEW_ELEMENT);
	...
	public String toXHTML(final RequestWrapper request, final IDataAccess dataAccess_, boolean submitted) throws DatabaseException {
		...
		try {
			...
			BasePCMServlet.log.log(Level.SEVERE, InternalErrorsConstants.XML_GRID_GENERATION, parqProsaExc);
