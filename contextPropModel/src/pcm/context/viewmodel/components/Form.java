...
import pcm.common.comparator.ComparatorFieldLogicSet;
import pcm.common.comparator.ComparatorFieldset;
import pcm.common.comparator.ComparatorIdButtons;
...
import pcm.comunication.actions.validators.IValidator;
import pcm.comunication.actions.validators.RelationalAndCIFValidator;
...
import pcm.context.viewmodel.components.controls.AbstractCtrl;
import pcm.context.viewmodel.components.controls.FieldsetControl;
import pcm.context.viewmodel.components.controls.ICtrl;
import pcm.context.viewmodel.components.controls.SelectCtrl;
...
public class Form extends AbstractComponent {
	...
	public Form(final String service_, final String event_, final Element formElement_, final IDataAccess dataAccess_,
			final ContextApp appCtx_, final RequestWrapper request_) throws PCMConfigurationException {
	...
	protected final void initFieldViewSets(final Element element, final RequestWrapper request, IDataAccess dataAccess_)
		...
		try {
			...
			final NodeList nodosUserButtons = element.getElementsByTagName(IViewMetamodel.USERBUTTONS_ELEMENT);
			...
			for (int i = 0; i < nodosLength; i++) {
				...
				if (userButtonsNode.getParentNode().getNodeName().equals(IViewMetamodel.FIELDSET_ELEMENT)) {
					// abrir fieldSet
					Element fieldSetOfUserButtons = ((Element) userButtonsNode.getParentNode());
					if (fieldSetOfUserButtons.hasAttribute(IViewMetamodel.LEGEND_ATTR)) {
						String legend = fieldSetOfUserButtons.getAttribute(IViewMetamodel.LEGEND_ATTR);
				...
				final NodeList nodosButton = userButtonsNode.getElementsByTagName(IViewMetamodel.BUTTON_ELEMENT);
				...
				for (int j = 0; j < nodosButtonLength; j++) {
					...
					final String name_ = button.getAttribute(IViewMetamodel.NAME_ATTR);
					final String name = Translator.traducePCMDefined(CommonUtils.getLanguage(request), name_);
					final String link = button.getAttribute(IViewMetamodel.LINK_ATTR);
					final String id = button.getAttribute(IViewMetamodel.ID_ATTR);
					final String onClickAttr = button.getAttribute(IViewMetamodel.ONCLICK_ATTR);
			...
			final NodeList nodosFieldViewSets = element.getElementsByTagName(IViewMetamodel.FIELDVIEWSET_ELEMENT);
			...
			for (int i = 0; i < nodosFieldViewLength; i++) {
				...
				final String nameSpace = fieldViewSetNode.getAttribute(IViewMetamodel.NAMESPACE_ENTITY_ATTR);
				
				String entityNameInMetamodel = fieldViewSetNode.getAttribute(IViewMetamodel.ENTITYMODEL_ATTR);
				final String persistToBBDD = fieldViewSetNode.getAttribute(IViewMetamodel.PERSIST_ATTR);
				final String order = fieldViewSetNode.getAttribute(IViewMetamodel.ORDER_ATTR);
				if (!"".equals(order)) {
				...
				NodeList nodosFieldViews = fieldViewSetNode.getElementsByTagName(IViewMetamodel.FIELDVIEW_ELEMENT);
				...
				coleccionFieldViews.addAll(obtenerColeccionFieldViews(listaFieldViewSets, fieldSetPrincipalCtrl, request, dataAccess_,
						...
						/* isButtonParam */fieldViewSetNode.getParentNode().getNodeName().equals(IViewMetamodel.BUTTON_ELEMENT)));
				...
				if (fieldViewSetNode.getParentNode().getNodeName().equals(IViewMetamodel.BUTTON_ELEMENT)) {
					final String id = ((Element) fieldViewSetNode.getParentNode()).getAttribute(IViewMetamodel.ID_ATTR);
			...
			BasePCMServlet.log.log(Level.SEVERE, "Internal- Exception when creating form component: ", ex22);
	...
	private final void generateComponentPattern(final IDataAccess dataAccess, final ICtrl fieldset, final IFieldView fieldView,
		...
		try {
			...
			BasePCMServlet.log.log(Level.SEVERE, "error gral.", exc);
		...
		try {
			...
			if (fieldView.getFieldAndEntityForThisOption().getEntityFromCharge() != null){
				final EntityLogic entidadCharger = EntityLogicFactory.getFactoryInstance().getEntityDef(
						getAppContext().getEntitiesDictionary(), fieldView.getFieldAndEntityForThisOption().getEntityFromCharge());
			...
			BasePCMServlet.log.log(Level.SEVERE, InternalErrorsConstants.FIELDVIEWSETS_CHARGE_OPTS_ERROR, cfgExc1);
			...
			BasePCMServlet.log.log(Level.SEVERE, InternalErrorsConstants.FIELDVIEWSETS_CHARGE_OPTS_ERROR, exc2);
	...
	public void bindUserInput(final IAction accion, final List<FieldViewSet> fieldViewSets, final List<MessageException> parqMensajes)
		...
		try {
			...
			BasePCMServlet.log.log(Level.SEVERE, InternalErrorsConstants.BINDING_ERROR, excc2);
	...
	public String toXHTML(final RequestWrapper request, final IDataAccess dataAccess_, boolean submitted) throws DatabaseException {
		try {
			...
			BasePCMServlet.log.log(Level.SEVERE, InternalErrorsConstants.XML_FORM_GENERATION, ez);
